...
import com.nicba1010.utils.JSONParser;
...
public class Chat extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sptag = bundle.getString("mobno") + "msgs";
		...
		if (bundle.getString("name") != null) {
			...
			// textview.setText(Html.fromHtml("<b>" + bundle.getString("name") +
			// " : </b>" + bundle.getString("msg")));
			// edit.putString(sptag, prefs.getString(sptag, "") +
			// (prefs.getString(sptag, "").length() > 0 ? separator : "") +
			// "<b>" + bundle.getString("name") + " : </b>" +
		...
		send_btn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				edit.putString(sptag, prefs.getString(sptag, "") + (prefs.getString(sptag, "").length() > 0 ? separator : "") + "<b>You : </b>" + chat_msg.getText().toString());
				edit.commit();
	...
	public void populate() {
		String messages_full = prefs.getString(sptag, "");
		if (messages_full.length() == 0) {
			return;
		} else if (!messages_full.contains(separator)) {
			TableRow tr1 = new TableRow(getApplicationContext());
			tr1.setLayoutParams(new TableRow.LayoutParams(TableRow.LayoutParams.WRAP_CONTENT, TableRow.LayoutParams.WRAP_CONTENT));
			TextView textview = new TextView(getApplicationContext());
			textview.setTextSize(20);
			textview.setTextColor(Color.parseColor(!messages_full.contains("<b>You : </b>") ? "#0B0719" : "#A901DB"));
			textview.setText(Html.fromHtml(messages_full));
			tr1.addView(textview);
			tab.addView(tr1);
		} else if (messages_full.contains(separator)) {
			for (String message : messages_full.split(separator)) {
				TableRow tr1 = new TableRow(getApplicationContext());
				tr1.setLayoutParams(new TableRow.LayoutParams(TableRow.LayoutParams.WRAP_CONTENT, TableRow.LayoutParams.WRAP_CONTENT));
				TextView textview = new TextView(getApplicationContext());
				textview.setTextSize(20);
				textview.setTextColor(Color.parseColor(!message.contains("<b>You : </b>") ? "#0B0719" : "#A901DB"));
	...
	private class Send extends AsyncTask<String, String, JSONObject> {
		...
		protected JSONObject doInBackground(String... args) {
			...
			JSONObject jObj = json.getJSONFromUrl("http://readysteadycode.tk:8070/send", params);
			return jObj;
		}
